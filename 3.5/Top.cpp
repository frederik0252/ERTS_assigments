...
Top::Top(sc_module_name name)
    : sc_module(name),
      ...
      master("master"), inAdapt("inAdapt"),
      slave("slave", "received_3_5.txt"), trace_file_(nullptr)
{
    // The FIFO interface is implemented directly by the adapter.
    master.out(inAdapt);
    inAdapt.clock(clock);
    inAdapt.reset(reset);
    inAdapt.ready(ready);
    ...
    // Exactly the same sink and backpressure pattern as Exercise 3.4.
    slave.clock(clock);
    slave.reset(reset);
    slave.ready(ready);
    slave.valid(valid);
    ...
    trace_file_ = sc_create_vcd_trace_file("tlm_bcam_3_5");
